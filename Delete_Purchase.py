...
class Delete_Purchase:
    def __init__(self,FindID):
        self.findid = FindID
        self.findid.title("Delete Purchase Record")
        self.suppframe = Frame(self.findid,relief=RAISED,borderwidth=1,pady=20)

        #add supplier id Label
        self.suppid = Label(self.suppframe,text="Purchase Number : ",padx=10,pady=10)
...
def run_delete_purchase():
    delete_purchase1 = Tk()
    Delete_Purchase(delete_purchase1)
    delete_purchase1.geometry("600x600+50+50")
    delete_purchase1.mainloop()
